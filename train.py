...
def main():
    ...
    if configurations.LOAD:
        utility.load_model(
            configurations.BEST_FILE, network, optimizer, configurations.LEARNING_RATE
        )
    ...
    for epoch in range(1, configurations.NUM_EPOCHS):
        ...
        if configurations.SAVE:
            utility.save_model(network, optimizer, configurations.CHECK_FILE_NAME)
            """
        ...
        if epoch % 5 == 0:
            # calculate accuracy
            train_accuracy = utility.calculate_accuracy(network, train_loader)
            print("current train accuracy at " + str(round(train_accuracy, 2)) + "%")
            test_accuracy = utility.calculate_accuracy(network, test_loader)
            print("current test accuracy at " + str(round(test_accuracy, 2)) + "%")
            ...
            if (
                ...
                and test_accuracy > configurations.BEST_SO_FAR
            ):
                utility.save_model(
                    network,
                    optimizer,
